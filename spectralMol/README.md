...
An evolutionary drug discovery framework that optimises molecules against multi-parameter oracles using a Fourier-genotype SpectralMol local optimizer, plus optional graph/SMILES benchmark baselines. Benchmarks are provided for the GuacaMol preset suite (via MolScore) and the SATURN docking-guided benchmark.
...
| `core.spectral_evolution` | Fourier-genotype evolutionary generator (`Theta -> Phi@Theta -> SELFIES -> SMILES`). |
...
| `--generator` | `spectral` | `spectral` evolves Fourier `Theta`; `rdkit_mutation` keeps the legacy graph/SMILES mutation path |
| `--frequency-mode` | `full-spectrum` | Spectral condition: `full-spectrum`, `high-only`, `low-only`, or `random-matrix` |
| `--spectral-l`, `--spectral-k`, `--spectral-d` | `32`, `16`, `32` | Decode length, Fourier harmonics, and embedding dimension |
| `--spectral-decode-attempts` | `8` | SELFIES decode attempts per `Theta` candidate |
...
| `--local-evo-generator` | `spectral` | Generator for local_evolution; use `rdkit_mutation` for the legacy graph/SMILES path |
| `--local-evo-frequency-mode` | `full-spectrum` | Fourier frequency condition for spectral local_evolution |
