...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default="",
        help=(
            "Comma-separated zero-based MolScore task indexes for builtin local_evolution models. "
            "Use this when MolScore's include filter cannot select a displayed task name."
        ),
    )
    ...
    p.add_argument(
        ...
        help="Generator used by builtin local_evolution.",
    )
    p.add_argument(
        "--local-evo-frequency-mode",
        choices=("full-spectrum", "high-only", "low-only", "random-matrix"),
        default=getattr(cfg, "FREQUENCY_MODE_DEFAULT", "full-spectrum"),
        help="Frequency condition for builtin spectral local_evolution.",
    )
    p.add_argument(
...
def parse_int_csv(raw: str) -> list[int]:
    ...
    for token in parse_csv_list(raw):
        try:
            ...
            raise ValueError(f"Expected integer task index, got {token!r}") from exc
        if idx < 0:
...
def render_command_template(
    ...
    *,
    ...
    task_indexes_args = f"--task-indexes {task_indexes_csv}" if task_indexes_csv else ""
    formatted = template.format(
        ...
        task_indexes_args=task_indexes_args,
        budget=budget,
...
def tail_text(path: Path, *, n_lines: int = 80) -> str:
    """Return a compact log tail for failure messages."""
    try:
...
def build_best_seed_tables(raw_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Summarize each task by the best seed/run instead of the mean over seeds."""
    if raw_df.empty:
        return pd.DataFrame(), pd.DataFrame()
    ...
    n_runs = (
        df.groupby(["model", "task"], as_index=False)["seed"]
        .count()
        .rename(columns={"seed": "n_runs"})
    )
    best_idx = df.groupby(["model", "task"])["best_score"].idxmax()
    task_best = df.loc[best_idx].copy()
    task_best = task_best.merge(n_runs, on=["model", "task"], how="left")
    task_best = task_best.rename(
        columns={
            "seed": "best_seed",
            "best_score": "best_seed_score",
            "avg_score": "best_seed_avg_score",
            "n_scored": "best_seed_n_scored",
        }
    )
    keep_cols = [
        "model",
    ...
    task_best = task_best[[c for c in keep_cols if c in task_best.columns]]
    task_best = task_best.sort_values(["task", "best_seed_score"], ascending=[True, False])

    overall_best = (
        task_best.groupby(["model"], as_index=False)
        .agg(
            sum_task_best_seed_score=("best_seed_score", "sum"),
            mean_task_best_seed_score=("best_seed_score", "mean"),
            n_tasks=("task", "count"),
            total_runs=("n_runs", "sum"),
        )
        .sort_values("sum_task_best_seed_score", ascending=False)
    )
    overall_best["rank"] = np.arange(1, len(overall_best) + 1)
    return task_best, overall_best
...
def main() -> int:
    ...
    task_indexes = parse_int_csv(args.task_indexes)
    include_csv = ",".join(include)
    exclude_csv = ",".join(exclude)
    task_indexes_csv = ",".join(str(idx) for idx in task_indexes)
    ...
    if args.local_evo_generator not in {"spectral", "rdkit_mutation"}:
        raise ValueError("--local-evo-generator must be spectral or rdkit_mutation")
    if args.local_evo_frequency_mode not in {"full-spectrum", "high-only", "low-only", "random-matrix"}:
        raise ValueError("--local-evo-frequency-mode is invalid")
    ...
    if task_indexes_csv:
        ...
        for model in selected_models:
            ...
            if spec is None or spec.type == "builtin_local_evolution":
                continue
            if spec.type != "command" or not spec.command or "{task_indexes_args}" not in spec.command:
                unsupported.append(model)
        if unsupported:
            raise ValueError(
                "--task-indexes is supported for builtin local_evolution models and command templates "
                f"with {{task_indexes_args}}; unsupported selected models: {','.join(unsupported)}"
            )
        ...
        for seed in seeds:
            ...
            if spec.type == "builtin_local_evolution":
                cmd = render_builtin_local_evolution_command(
                    ...
                    generator=args.local_evo_generator,
                    frequency_mode=args.local_evo_frequency_mode,
    ...
    run_meta = {
        ...
        "smiles_rnn_device": smiles_rnn_device,
        "local_evo_generator": str(args.local_evo_generator),
        "local_evo_frequency_mode": str(args.local_evo_frequency_mode),
        ...
        if rc != 0:
            return (
                ...
                0,
                {
                    ...
                    "seed": seed,
                    "error": (
                        f"Command failed with return code {rc}. See log: {log_path}\n"
                        f"--- log tail ({log_path}) ---\n{tail_text(log_path)}"
                    ),
                },
                runtime_row,
    ...
    task_best, overall_best = build_best_seed_tables(raw_df)
    task_best_path = out_root / "comparison_task_best_seed_summary.tsv"
    overall_best_path = out_root / "comparison_overall_best_seed.tsv"
    task_best.to_csv(task_best_path, sep="\t", index=False)
    overall_best.to_csv(overall_best_path, sep="\t", index=False)
    ...
    print(f"Task best-seed:   {task_best_path}")
    print(f"Overall best-seed:{overall_best_path}")
