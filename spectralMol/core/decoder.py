...
from .vocab import MACRO_EXPANSIONS, macro_atom_count, merge_macro_expansions
...
class DecodeTokenState:
    ...
    macro_atom_counts: Dict[str, int]
...
def _freeze_macro_expansions_for_cache(macro_expansions):
    if not macro_expansions:
        return ()
    frozen = []
    for key, value in macro_expansions.items():
        if isinstance(value, (list, tuple)):
            value_sig = tuple(str(tok) for tok in value)
        else:
            value_sig = (str(value),)
        frozen.append((str(key), value_sig))
    return tuple(sorted(frozen))


def build_decode_token_state(
    id2tok: dict[int, str],
    macro_expansions: Optional[Mapping[str, str]] = None,
) -> DecodeTokenState:
    ...
    macro_signature = _freeze_macro_expansions_for_cache(macro_expansions)
    cache_key = (id(id2tok), allowed_elems, macro_allow_cfg, macro_deny_cfg, macro_signature)
    ...
    macro_tokens = set(all_macro_expansions.keys())
    atom_tokens = {t for t in set(id2tok.values()) if atom_any_re.match(t)}
    macro_atom_counts = {name: macro_atom_count(expansion) for name, expansion in all_macro_expansions.items()}
    ...
    state = DecodeTokenState(
        ...
        macro_expansions=dict(all_macro_expansions),
        macro_atom_counts=macro_atom_counts,
    )
...
def ids_to_selfies(
    ...
    expansions = merge_macro_expansions(macro_expansions)
    toks: list[str] = []
...
def dist2_to_ids_sample(
    ...
    macro_atom_counts = state.macro_atom_counts
...
def decode_theta_best_of_k(
    ...
    *,
    ...
    if token_state is not None:
        macro_tokens = set(token_state.macro_tokens)
        active_macro_expansions = token_state.macro_expansions
    else:
        macro_tokens = set(MACRO_EXPANSIONS.keys())
        active_macro_expansions = MACRO_EXPANSIONS
    ...
    def _consider_ids(ids: np.ndarray, *, count_decoder_fail: bool = False) -> None:
        nonlocal best_smi, best_f, best_macro_count, decoder_fail_count
        ...
        selfies = ids_to_selfies(ids, id2tok=id2tok, macro_expansions=active_macro_expansions)
        if not selfies:
        ...
        if canon is None:
            ...
            if count_decoder_fail and r == "SELFIES2SMILES_FAIL":
    ...
    # ------------------------------------------------------------
    # Optional direct decode of the nearest token at each position.
    # This is useful when Theta was produced by encoding a known target:
    # the stochastic/ring-plan attempts below are exploration-biased and can
    # otherwise skip the most direct projection represented by Theta.
    # ------------------------------------------------------------
    if bool(getattr(CFG, "DECODE_INCLUDE_GREEDY_NEAREST", False)):
        if dist2_topk_ids_precomputed is not None and dist2_topk_vals_precomputed is not None:
            nearest_idx = np.argmin(dist2_topk_vals_precomputed, axis=1)
            rows = np.arange(int(dist2_topk_ids_precomputed.shape[0]))
            ids = dist2_topk_ids_precomputed[rows, nearest_idx].astype(np.int64, copy=False)
        else:
            assert dist2 is not None
            ids = np.argmin(dist2, axis=1).astype(np.int64, copy=False)
        _consider_ids(ids)
    ...
    for _ in range(max(0, k - 1)):
        ...
        _consider_ids(ids, count_decoder_fail=True)
