...
import os
import re
from typing import Dict, Set, List, Optional, Iterable, Mapping
...
MACRO_EXPANSIONS: Dict[str, str] = {
    ...
    # --- Saturn / ClpP docking motifs ---
    #
    # The Saturn table-8 task rewards low docking energies while retaining QED
    # and SA. These motifs are intentionally modest phenyl/amide/ether handles:
    # they help theta-only edits stay near the high-docking seed chemotypes
    # without introducing phenotype-level fragment operations.
    "[PHENETHYL]":       "[c][c][c][c][c][c][Ring5][C][C]",
    "[PHENPROPYL]":      "[c][c][c][c][c][c][Ring5][C][C][C]",
    "[F_PHENYL]":        "[c][c][c][c][c][c][Ring5][Branch1][F]",
    "[CL_PHENYL]":       "[c][c][c][c][c][c][Ring5][Branch1][Cl]",
    "[BR_PHENYL]":       "[c][c][c][c][c][c][Ring5][Branch1][Br]",
    "[F_PHENETHYL]":     "[c][c][c][c][c][c][Ring5][Branch1][F][C][C]",
    "[CL_PHENETHYL]":    "[c][c][c][c][c][c][Ring5][Branch1][Cl][C][C]",
    "[BR_PHENETHYL]":    "[c][c][c][c][c][c][Ring5][Branch1][Br][C][C]",
    "[BENZAMIDE]":       "[c][c][c][c][c][c][Ring5][C][Branch1][=O][N]",
    "[PHENACETAMIDE]":   "[c][c][c][c][c][c][Ring5][C][C][Branch1][=O][N]",
    "[BENZYL_AMIDE]":    "[c][c][c][c][c][c][Ring5][C][N][C][Branch1][=O]",
    "[PHENETHYL_AMIDE]": "[c][c][c][c][c][c][Ring5][C][C][N][C][Branch1][=O]",
    "[DIARYL_AMIDE]":    "[c][c][c][c][c][c][Ring5][C][Branch1][=O][N][c][c][c][c][c][c][Ring5]",
    "[PHENOXY_ETHYL]":   "[c][c][c][c][c][c][Ring5][O][C][C]",
    "[BENZYL_ETHER]":    "[c][c][c][c][c][c][Ring5][C][O]",
}


def _macro_expansion_to_selfies(expansion) -> str:
    if isinstance(expansion, (list, tuple)):
        return "".join(str(tok) for tok in expansion)
    return str(expansion or "")


# Optional Saturn docking-oriented macro motifs.
# These are expanded into existing primitive SELFIES tokens, so theta remains
# the only evolved object while the representation can expose useful handles.
_SATURN_DEEPDOCK_MACROS = {
    "[SATURN_PHENYL_ETHER]": ["[c]", "[c]", "[c]", "[c]", "[c]", "[c]", "[Ring5]", "[O]"],
    "[SATURN_BENZAMIDE]": ["[c]", "[c]", "[c]", "[c]", "[c]", "[c]", "[Ring5]", "[C]", "[Branch1]", "[=O]", "[N]"],
    "[SATURN_PHENETHYL]": ["[c]", "[c]", "[c]", "[c]", "[c]", "[c]", "[Ring5]", "[C]", "[C]"],
    "[SATURN_PHENETHYL_AMIDE]": ["[c]", "[c]", "[c]", "[c]", "[c]", "[c]", "[Ring5]", "[C]", "[C]", "[N]", "[C]", "[Branch1]", "[=O]"],
    "[SATURN_DIARYL_AMIDE]": ["[c]", "[c]", "[c]", "[c]", "[c]", "[c]", "[Ring5]", "[C]", "[Branch1]", "[=O]", "[N]", "[c]", "[c]", "[c]", "[c]", "[c]", "[c]", "[Ring5]"],
    "[SATURN_DIARYL_ETHER]": ["[c]", "[c]", "[c]", "[c]", "[c]", "[c]", "[Ring5]", "[O]", "[c]", "[c]", "[c]", "[c]", "[c]", "[c]", "[Ring5]"],
    "[SATURN_PHENPROPIONAMIDE]": ["[c]", "[c]", "[c]", "[c]", "[c]", "[c]", "[Ring5]", "[C]", "[C]", "[C]", "[Branch1]", "[=O]", "[N]"],
    "[SATURN_NNO_HETERO]": ["[C]", "[=C]", "[C]", "[=N]", "[N]", "[C]", "[C]", "[N]", "[O]", "[C]", "[C]", "[C]", "[=C]", "[Ring1]", "[=N]"],
    "[SATURN_AMINO_NNO_HETERO]": ["[N]", "[C]", "[=N]", "[N]", "[C]", "[C]", "[N]", "[O]", "[C]", "[C]", "[C]", "[=C]", "[C]", "[=C]", "[Ring1]", "[=N]"],
}
if os.environ.get("SPECTRAL_ENABLE_SATURN_DEEPDOCK_MACROS", "0").strip().lower() in {"1", "true", "yes", "on"}:
    MACRO_EXPANSIONS.update(
        {k: _macro_expansion_to_selfies(v) for k, v in _SATURN_DEEPDOCK_MACROS.items() if k not in MACRO_EXPANSIONS}
    )


# Optional generic motifs observed across the Saturn Table 8 hit families.
# These are not exact benchmark molecules; they expose compact medicinal
# handles (aryl ureas, piperazines, fluorophenyls, cyclopropyl amides) as
# theta-addressable SELFIES tokens for the Saturn representation only.
_SATURN_TABLE8_MACROS = {
    "[SATURN_T8_PIPERAZINE]": "[N][C][C][N][C][C][Ring1][=Branch1]",
    "[SATURN_T8_MORPHOLINE]": "[O][C][C][N][C][C][Ring1][=Branch1]",
    "[SATURN_T8_ARYL_UREA]": "[C][=C][C][=C][C][=C][Ring1][=Branch1][N][C][=Branch1][C][=O][N]",
    "[SATURN_T8_BENZYL_UREA]": "[C][=C][C][=C][C][=C][Ring1][=Branch1][C][N][C][=Branch1][C][=O][N]",
    "[SATURN_T8_PHENYL_PIPERAZINE]": "[C][=C][C][=C][C][=C][Ring1][=Branch1][N][C][C][N][C][C][Ring1][=Branch1]",
    "[SATURN_T8_FLUOROPHENYL]": "[F][C][=C][C][=C][C][=C][Ring1][=Branch1]",
    "[SATURN_T8_FLUOROBENZYL]": "[F][C][=C][C][=C][C][=C][Ring1][=Branch1][C]",
    "[SATURN_T8_CYCLOPROPYL_AMIDE]": "[C][C][C][Ring1][Ring1][C][=Branch1][C][=O][N]",
    "[SATURN_T8_PHENYL_CYCLOPROPYL]": "[C][=C][C][=C][C][=C][Ring1][=Branch1][C][C][C][Ring1][Ring1]",
    "[SATURN_T8_ARYL_PIPERAZINE_UREA]": "[O][=C][N][C][=C][C][=C][C][=C][Ring1][=Branch1][N][C][C][N][C][C][Ring1][=Branch1]",
    "[SATURN_T8_BENZAMIDE_OCH2_CF3ARYL]": "[N][C][=Branch1][C][=O][C][=C][C][=C][C][=C][Ring1][=Branch1][O][C][C][=C][C][=C][Branch1][=Branch2][C][Branch1][C][F][Branch1][C][F][F][C][=C][Ring1][#Branch2]",
    "[SATURN_T8_BENZAMIDE_CH2_CF3ARYL]": "[N][C][=Branch1][C][=O][C][=C][C][=C][C][=C][Ring1][=Branch1][C][C][C][=C][C][=C][C][Branch1][=Branch2][C][Branch1][C][F][Branch1][C][F][F][=C][Ring1][#Branch2]",
    "[SATURN_T8_PHENETHYL_PIPERAZINE_UREA]": "[O][=C][Branch1][N][N][C][C][C][=C][C][=C][C][=C][Ring1][=Branch1][N][C][C][N][Branch1][N][C][=C][C][=C][Branch1][C][Cl][C][=C][Ring1][#Branch1][C][C][Ring1][=N]",
    "[SATURN_T8_INDOLINONE_OCH2_CF3ARYL]": "[O][=C][N][C][=C][C][=C][C][Branch2][Ring1][Branch1][O][C][C][=C][C][=C][C][Branch1][=Branch2][C][Branch1][C][F][Branch1][C][F][F][=C][Ring1][#Branch2][=C][Ring2][Ring1][C][Ring2][Ring1][Ring2]",
    "[SATURN_T8_ISOCYANATE]": "[C][=C][=N]",
    "[SATURN_T8_N_ISOCYANATE]": "[N][=C][=C]",
    "[SATURN_T8_KETENE]": "[C][=C][=O]",
}
if os.environ.get("SPECTRAL_ENABLE_SATURN_TABLE8_MACROS", "0").strip().lower() in {"1", "true", "yes", "on"}:
    MACRO_EXPANSIONS.update(
        {k: _macro_expansion_to_selfies(v) for k, v in _SATURN_TABLE8_MACROS.items() if k not in MACRO_EXPANSIONS}
    )
...
def macro_atom_count(expansion: str) -> int:
    """Count atom-like tokens in an expansion (excluding Branch/Ring control tokens)."""
    expansion = _macro_expansion_to_selfies(expansion)
    n = 0
...
def macro_is_aromatic(expansion: str) -> bool:
    """Heuristic: expansion contains aromatic atom tokens."""
    expansion = _macro_expansion_to_selfies(expansion)
    return any(t in expansion for t in ("[c]", "[n]", "[o]", "[s]"))


def merge_macro_expansions(extra: Optional[Mapping[str, str]] = None) -> Dict[str, str]:
    """Return static macros plus optional runtime macro expansions."""
    merged = dict(MACRO_EXPANSIONS)
    if extra:
        for name, expansion in extra.items():
            if name and expansion:
                merged[str(name)] = _macro_expansion_to_selfies(expansion)
    return merged

# These names are imported by other modules (embedding.py, decoder.py).
MACRO_ATOM_COUNTS: Dict[str, int] = {k: macro_atom_count(v) for k, v in MACRO_EXPANSIONS.items()}
MACRO_IS_AROM: Dict[str, bool] = {k: macro_is_aromatic(v) for k, v in MACRO_EXPANSIONS.items()}
...
def build_vocab_from_semantic_robust_alphabet(
    *,
    ...
    dynamic_macro_expansions: Optional[Mapping[str, str]] = None,
    ...
    macro_expansions = merge_macro_expansions(dynamic_macro_expansions)
